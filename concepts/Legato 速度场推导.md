...
> [!note] 先厘清定位：这是「方法迁移」链接，不是「同主题」
> **Legato 本身不是 RL**——没有 reward / value / policy improvement，它主题归属在 [[推理动力学 主题地图]]（§3.4b），我当初也是因为推理动力学才读它。它进入我 RL 思路的**唯一身份是「方法模板」**：我借的是"反解范式"这个**手法**，不是它的内容——跟 [[LLM 启发地图]] 里"从 LLM 借技术到 VLA"是同一种**横切链接**，不是把它当竞品。
> **而真正的研究问题就藏在这次迁移里**：Legato 反解之所以干净，是因为目标 $\mathbf{A}$（已知前缀）是**给定、固定**的；可我要反解的 advantage / Q 引导依赖一个**训练中变动、有偏、学出来的 critic** → "训练=推理"的保证会退化成"对当前 value 估计一致"，还接回 critic 偏差 / off-policy / target 漂移这些 Legato 从不面对的硬骨头。**这个"从已知前缀跳到学出来的价值"的落差，就是我论文里那个不平凡的适配 = 贡献与风险所在。**
