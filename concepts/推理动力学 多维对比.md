...
**改推理 vs 改训练（最重要的分界）。** 纯改推理（training-free、权重不动）：DDIM、RTC、PD-VLA、FLASH（draft头另训但base冻结）。纯/重改训练目标：DDPM、Diffusion Policy、π₀、Legato、Consistency Policy/OneDP/ManiCM/FlowPolicy（蒸馏/一致性把新目标写进训练）。"两者皆动"的（训练加机制、收益落推理）：SDP、Streaming Flow Policy、Discrete Diffusion VLA、VLASH、DynamicVLA。决策含义：要不动现有模型直接提速选 training-free；要根治多模态/衔接质量必须动训练。
...
- **代码未公开**：Legato（项目页无GitHub,全部机制来自arXiv HTML/PDF;推理步数N=5不确定、κ=ω/Δt等符号细节可能有偏差）、RTC（未clone,论文/检索级;soft mask衰减曲线、d取值、clamp vs gradient实现未核实）。
