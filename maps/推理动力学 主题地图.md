...
> 关联：[[paper-list]]（阅读队列）｜[[强化学习后训练 主题地图]]（上层主题）｜[[推理动力学 多维对比]]（逐维拆开比异同，精读级）｜标签：`#inference-dynamics`
...
| **Legato**（Learning Native Continuation） | RTC 的衔接只在**推理时**做 inpainting 约束、不进策略参数；Legato 把 chunk 衔接做成策略的**原生能力**——训练时用 schedule-shaped per-step continuation **重塑 flow 速度场**，让训练分布与推理时的续接执行对齐，减少 chunk 边界的多模态跳变（更少犹豫、动作模式更一致）。5 个真机任务上轨迹平滑度与完成时间各约 +10%，超过 RTC。**RTC 的训练时对应方案** | 前沿（2026.02） | 未读 | [2602.12978](https://arxiv.org/abs/2602.12978) |
