...
> [!tip] 这是**工作层（skim）**，不是深读清单
> 本图职责 = 广度扫描，读到能下工程判断（"何时用 / 代价多大"）就合上，不逐篇深究。
> 想 **focus 深读**（我自己要做的那个点）→ 看 [[我的研究主线（flow v_target 价值注入）]]。两层别混：用深读标准读这 30 篇会永远读不完。
> 关联：[[paper-list]]（阅读队列）｜[[推理动力学 主题地图]]（**正交邻居**：与 RL 的交集 = chunk 级/异步信用分配，非底座）｜[[Diffusion-Flow 策略的 RL 改进路线图]]（§4 对照轴的细化：四条路线框架 + 检索存档）｜[[机器人学习的数据来源 主题地图]]（数据维度的姊妹图——算法这边问"怎么学"，那边问"用什么数据"）｜标签：`#RL` `#post-training`
...
> [!note] 邻域参照（非本图核心，人形控制侧）
> 「示范/参考数据怎么用」这对张力的上位抽象见概念笔记 [[参考运动作为塑形先验]]——**硬跟踪约束 vs 软塑形先验**。人形全身控制的 [[参考运动作为先验的人形 RL（HIL × Multi-Task Reference RL）|HIL × Multi-Task Ref RL]] 是该概念的两个实例（多任务联合 + 共享观测）；本图的 [[Advantage-conditioned 微调]] 是其 VLA 侧的另一种实现。它们不是 VLA/manipulation 竞品，但能反向照亮「怎么不破坏预训练」。
...
> 按**算法范式**切（同一篇可能跨维度——见图下跨角度说明与 §4 对照轴）；**正交的"RL 信号从哪个口子进策略"分类**见 [[Diffusion-Flow 策略的 RL 改进路线图]]。④ 是与**世界模型**这个独立邻居的**交集切面**，不是 RL 的子分支。
...
flowchart TD
    ...
    ROOT --> S4["④ imagination 驱动 RL<br/>(=世界模型∩RL 交集)"]
    ROOT --> S5["⑤ 样本高效真机 / HIL"]
    ROOT --> S6["⑥ latent reasoning RL"]

    S1 --> S1P["π_RL · VLA-RL · RIPT-VLA<br/>SimpleVLA-RL · PLD · iRe-VLA · RL Token<br/>latent steering: DSRL · ZPRL · RFS · LPS"]
    S2 --> S2P["LWD · ConRFT<br/>RL-100 · CO-RFT"]
    S3 --> S3P["QAM · Q-chunking · Decoupled Q-Chunking<br/>TGRPO · VLAC · VL-DAC"]
    S4 --> S4P["RISE ✅ · WMPO · VLA-RFT<br/>DiWA · SC-VLA"]
    S5 --> S5P["ROVE · ConRFT · VLAC · RL-100<br/>HIL-SERL（背景）"]
...
| **π*₀.₆**（即 RECAP 论文） | "从经验中学习"的 VLA；RECAP 是其中的 advantage-conditioned 方法，RISE 继承并修正了它。其理论一般式（β=guidance scale 可调）见 [[Advantage-conditioned 微调]] 的 CFGRL | 核心 | ⬜未读 | [2511.14759](https://arxiv.org/abs/2511.14759) |
...
**3.1b latent-space RL steering**

> 一个独立小类：RL **不直接微调权重、也不在动作空间做残差**，而是在隐空间 / 噪声空间 / bottleneck 空间里学，去 steer 一个（通常冻结的）预训练策略。

| 文章 | 一句话作用 | 相关度 | 状态 | arXiv |
| :--- | :--- | :--- | :--- | :--- |
| **DSRL** | RL 学在扩散策略的**潜噪声空间**——二级策略选噪声向量喂给冻结的扩散策略，只需黑盒访问。本类范式起点 | 核心 | ⬜未读 | [2506.15799](https://arxiv.org/abs/2506.15799) |
| **ZPRL** | 训练时加变分信息瓶颈（VIB），在线时冻结基础策略、RL 在 **bottleneck latent** 做 Z-perturbation。明确反对动作残差。清华 Huazhe Xu 组 | 核心 | ⬜未读 | [2605.19919](https://arxiv.org/abs/2605.19919) |
| **RFS** | flow-matching 策略适配：**联合**优化动作残差 + 潜噪声分布——局部细化靠残差、全局探索靠 latent modulation。灵巧操作 | 核心 | ⬜未读 | [2602.01789](https://arxiv.org/abs/2602.01789) |
| **LPS** | offline-RL 分支：原动作空间的 Q-gradient 经可微的一步 flow 策略反传，更新**潜动作空间**的 actor | 相关 | ⬜未读 | [2603.05296](https://arxiv.org/abs/2603.05296) |

### 3.2 ② offline-to-online RL

| 文章 | 一句话作用 | 相关度 | 状态 | arXiv |
| :--- | :--- | :--- | :--- | :--- |
| **LWD**（Learning while Deploying） | 舰队规模 offline-to-online RL：真机舰队部署数据 + DIVL（Distributional Implicit Value Learning，分布式隐式价值学习）+ QAM 策略抽取 | 核心 | 📥队列中 | [agibot](https://finch.agibot.com/research/lwd) |
...
### 3.4 ④ imagination 驱动的 RL 后训练（= 世界模型 ∩ RL 的交集）

> [!warning] 边界：这**不是**"世界模型作为 RL 的子分支"
> **世界模型是独立的平级邻居主题**（表示学习 / 视频预测 / 规划，大部分与 RL 无关），主体见 [[世界模型 主题地图]]。本节只收它与 RL 的**唯一交集切面**——"想象当 RL 的价值 / 数据来源"。**纯世界模型工作不要往这里塞。**
> 跟我已读的 RISE 直接对照，也连着 [[Compositional World Model]] 概念笔记。
...
> 主力 **ConRFT（§3.2）/ VLAC（§3.3）/ RL-100（§3.2）** 都同属此类（「真机 + 人类干预 + 少量 episode」），主归属在前面、这里不重复列。本节只收两篇**本节专属**的：新核心实例 **ROVE** + 奠基背景 **HIL-SERL**。

| 文章 | 一句话作用 | 相关度 | 状态 | arXiv |
| :--- | :--- | :--- | :--- | :--- |
| **ROVE** | 把人类介入当「信号」而非专家标签：用 **乐观价值估计 OVE**（H-step TD + expectile τ=0.7）从混杂质量的介入轨迹里筛高价值恢复行为，再 advantage-conditioned BC 蒸馏进 flow-matching VLA；引入跨本体人类第一视角视频补长尾失败/恢复态价值。critic 初始化自 §3.3 的 **VLAC**，policy 用 Qwen3-VL-4B + flow DiT。直接对照 [[Advantage-conditioned 微调]]、呼应我的 [[flow-vtarget-advantage-idea]] | 核心 | ⬜未读 | [2606.17011](https://arxiv.org/abs/2606.17011) |
| **HIL-SERL** | human-in-the-loop 样本高效真机 RL 的奠基工作（非 VLA 本体），是 ⑤ 这条线的源头 | 相关 · 背景 | ⬜未读 | [2410.21845](https://arxiv.org/abs/2410.21845) |
...
| **State Representation Learning for DRL**（综述） | RL 中 state 表示学习的系统综述——把 state 放在比 reward 更核心的位置（state-centric RL）；SRL 是 RSSM / JEPA / contrastive / bisimulation 等的共同上位词 | 综述 | 📥队列中 | [2506.17518](https://arxiv.org/abs/2506.17518) |
...
| 最小侵入（冻结 + 小 head / residual / latent steering） | RL Token · PLD · DSRL · ZPRL · RFS | 低 |

> 注：此表按"信号来源 × 工程侵入度"切；"最小侵入"行里 DSRL（②噪声空间）与 RL Token（①输出端重排）机制不同，只是恰好都低侵入——**机制级的"四口子"分类**见 [[Diffusion-Flow 策略的 RL 改进路线图]]。
...
> _现在别急着下结论，先读。_
>
> ✅ **2026-07-13 已回填**：切入点定了——[[研究主线：质量感知的 Advantage-conditioned 后训练]]（准则改为「首先有效，其次新颖」；直接回应提示方向①，宿主方法 RECAP 正是 `~/recap` 在实现的东西；撞车检查通过，头号近邻 ROVE 已登记 P0）。

---

## 5. 待补 / 存疑

- [x] **HIL-SERL 的 arXiv 号已核实** —— 2410.21845 正确（Science Robotics 2025 有正式版）
